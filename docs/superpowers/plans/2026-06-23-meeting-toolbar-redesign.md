...
- [x] `폰트 > 크기 > 굵게 > 글자색 > 배경색 > 목록 > 정렬` 순서로 툴바 그룹을 재배치한다.
- [x] 보조 기능은 시각적 강조를 낮추거나 뒤로 보낸다.
...
- [x] 버튼 높이, 패딩, 테두리, 배경을 줄여서 툴바를 더 얇고 가볍게 만든다.
- [x] 버튼 라벨과 선택 컨트롤 스타일을 HWP/DOCX 계열 문서앱처럼 평평하고 읽기 쉽게 맞춘다.
- [x] 그룹 구분은 과한 캡슐형 대신 약한 구분선과 간격으로 처리한다.
...
- [x] `좌/중/우` 같은 투박한 표현을 더 익숙한 라벨 또는 아이콘 느낌으로 조정한다.
- [x] 자주 쓰는 기능이 첫 시선에 읽히도록 순서와 시각 위계를 맞춘다.
- [x] 기존 저장, 불러오기, 동기화 동작에 영향이 없는지 확인한다.
